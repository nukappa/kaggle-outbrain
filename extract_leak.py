...
memory = 100 # stands for 10GB, write your memory here
limit = 114434838 / 10 * memory 

leak = {}
for c,row in enumerate(csv.DictReader(gzip.open('../raw/promoted_content.csv.gz', 'rt'))):
    if row['document_id'] != '':
        leak[row['document_id']] = 1 
print(len(leak))
count = 0
filename = '../raw/page_views.csv.gz'
for c,row in enumerate(csv.DictReader(gzip.open(filename, 'rt'))):
    if count>limit:
	    break
    if c%1000000 == 0:
        print (c,count)
